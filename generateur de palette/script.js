...
function setup() {
  ...
  background(255);
  noStroke(); // Supprime les contours des triangles
  ...
  // Étape 3 : Dessiner les triangles imbriqués
  drawNestedTriangles(chosenColors);

  // Étape 4 : Afficher les concepts et les détails des couleurs
  displayDetails(chosenSubConcepts, colorDetails);
}

// Affiche les concepts choisis et les couleurs associées
function displayDetails(subConcepts, details) {
  fill(0);
  textSize(16);
  textAlign(CENTER, CENTER);
  text(`Concepts sélectionnés : ${subConcepts.join(", ")}`, width / 2, height - 100);
  textSize(12);
  text(details.join("\n"), width / 2, height - 50);
}

// Dessiner les triangles imbriqués
function drawNestedTriangles(colors) {
  let centerX = width / 2;
